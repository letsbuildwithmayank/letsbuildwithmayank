<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=180&section=header&text=Build%20With%20Mayank&fontSize=42&fontAlignY=35" width="100%"/>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1000&center=true&vCenter=true&width=600&lines=Web+%26+Android+Developer;Building+AI-powered+Apps+%26+Tools" />
</p>

<p align="center">
  <a href="mailto:letsbuildwithmayank@gmail.com"><img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>
</p>

## 👋 About Me
- 🎓 B.Tech in Artificial Intelligence & Machine Learning, COLLEGE_NAME (YEAR)
- 💻 Web & Android developer who builds complete, working products
- 🤖 I combine AI/ML with web and mobile apps to solve real business problems
- 💼 Open for freelance projects

## 🚀 What I Can Build
- 🌐 **Websites:** business sites, landing pages, portfolios
- 🛒 **Web Apps:** booking systems, ordering systems, dashboards, admin panels
- 📱 **Android Apps:** apps for businesses, students and everyday use
- 🤖 **AI/ML Solutions:** image analysis, prediction models, smart reports
- 💬 **AI Chatbots:** chatbots for websites and customer support
- ⚙️ **Automation:** tools that save time on repetitive work
- 🔧 **Fixes & Updates:** bug fixing and new features in existing projects

## 🛠️ Tech Stack
<p align="center">
  <img src="https://skillicons.dev/icons?i=html,css,js,react,nodejs,tailwind,python,tensorflow,sklearn,opencv,androidstudio,firebase,mongodb,git,github,vercel" />
</p>

## 📊 GitHub Stats
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=letsbuildwithmayank&show_icons=true&theme=tokyonight&hide_border=true" height="165"/>
  <img src="https://streak-stats.demolab.com?user=letsbuildwithmayank&theme=tokyonight&hide_border=true" height="165"/>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=letsbuildwithmayank&color=blueviolet" />
</p>

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=100&section=footer" width="100%"/>
